...
function CreateRoute() {
  ...
  return (
    <section className="mx-auto flex w-full max-w-xl flex-col gap-4 rounded-3xl border border-white/15 bg-black/20 p-6 text-white backdrop-blur">
      ...
      {viewModel.errorMessage && <StatusLine tone="error">{viewModel.errorMessage}</StatusLine>}
      ...
      >
        ...
        {currentStep === "share" && (
          <div className="mt-3 flex flex-col gap-3">
            ...
            {shareableLink && (
              <Button className="w-full" variant="ghost" onPress={handleCopyLink}>
            ...
            <div>
              <label className="block text-sm font-semibold text-white/70" htmlFor="answer-code">
                相手の応答コード
              </label>
              <textarea
                id="answer-code"
