...
describe("evaluator", () => {
  ...
  test("prefers corner ownership over non-corner ownership", () => {
    ...
    expect(evaluate(cornerBoard, "black")).toBeGreaterThan(evaluate(nonCornerBoard, "black"));
  });
  ...
  test("rewards mobility advantage", () => {
    ...
    expect(evaluate(mobilityBoard, "black")).toBeGreaterThan(evaluate(mobilityBoard, "white"));
  });
});
